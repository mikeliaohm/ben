...
from .card_stats import PlayerPosition, CardTrick, CardPlayed, CardDist, append_trick, print_deque
...
class GenericAgent:
    ...
    def set_init_x_play(self, public_hand_str: str, contract: str, declarer: int) -> None:
        ...
        """
        self.__declarer__ = PlayerPosition(declarer)
    ...
    def set_real_card_played(self, opening_lead52: int, player_i: int) -> None:
        seat = ((self.__declarer__.value + 1) % 4 + player_i) % 4
    ...
    def set_card_played(self, trick_i: int, leader_i: int, i: int, card: int) -> None:
        ...
        player_i = self.__position__.value - ((self.__declarer__.value + 1) % 4)
        if player_i < 0:
            player_i += 4
