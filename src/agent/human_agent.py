...
class HumanAgent(GenericAgent):
    def input_card(self, current_trick52: List[int] = None) -> None:
        while True:
            ...
            if self.validate_card(card):
                ...
                if current_trick52 is not None and len(current_trick52) > 0:
                    suit = current_trick52[0] // 13
                    if card_idx // 13 != suit and any(card // 13 == suit for card in self.__cards__):
    ...
    async def async_play_card(self, trick_i: int, leader_i: int, 
        ...
        card_idx = self.input_card(current_trick52)
        return CardResp(card=Card.from_code(card_idx), candidates=[], samples=[], shape=-1, hcp=-1, quality=None, who=None)
